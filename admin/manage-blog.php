...
if (isset($_GET['id']) && $_GET['id'] != '') {
    $id = $_GET['id'];
    $image_required = '';
$display_value = "SELECT * from blog where id ='$id'";
...
if (isset($_POST['save'])) {
    ...
    $a_name = get_safe_value($conn,$_POST['a_name']);
    $a_desi = get_safe_value($conn,$_POST['a_desi']);
    $a_desc = get_safe_value($conn,$_POST['a_desc']);
    $short_desc = get_safe_value($conn,$_POST['short_desc']);
    $b_desc = get_safe_value($conn,$_POST['b_desc']);
    $date = get_safe_value($conn,$_POST['date']);
    
        if (isset($_GET['id']) && $_GET['id'] != '') {
            if ($_FILES['image']['name'] == "") {
                $update_sql = "UPDATE `blog` SET `title`='$title',`short_desc`='$short_desc',`b_desc`='$b_desc',`date`='$date',`category`='$categories',`a_name`='$a_name',`a_desi`='$a_desi',`a_desc`='$a_desc' WHERE id ='$id'";
    ...
    $update_sql = "UPDATE `blog` SET `image`='$image',`title`='$title',`short_desc`='$short_desc',`b_desc`='$b_desc',`date`='$date',`category`='$categories',`a_name`='$a_name',`a_desi`='$a_desi',`a_desc`='$a_desc' WHERE id ='$id'";;
    $result = mysqli_query($conn,$update_sql);
}
}     
        else {
                $image = rand (111111111,999999999).'_'.$_FILES['image']['name'];
                move_uploaded_file($_FILES['image']['tmp_name'],PRODUCT_IMAGE_SERVER_PATH.$image);
    $insert_sql = "INSERT INTO `blog`(`image`, `title`, `short_desc`, `b_desc`, `date`, `category`, `a_name`, `a_desi`, `a_desc`) VALUES ('$image','$title','$short_desc','$b_desc','$date','$categories','$a_name','$a_desi','$a_desc')";
...
<div class="add-edit-product-form">
        <h4 class="title">About Blog</h4>
        <div class="row">
            <div class="col-lg-6 col-12 mb-30">
            <label for="title"><b>Title</b></label>    
            <input class="form-control" id='title' name='title' type="text" value="<?php
            if (isset($_GET['id']) && $_GET['id'] != '') {
            echo $row_display['title'];
            } 
            ?>">
        </div>
            <div class="col-lg-6 col-12 mb-30">
            <label for=""><b>Author Name </b></label>    
                <input class="form-control" type="text" name='a_name' value="<?php 
                if (isset($_GET['id']) && $_GET['id'] != '') {
                echo $row_display['a_name']; 
                }
                ?>">
            </div>
            
            <div class="col-lg-6 col-12 mb-30">
            <label for="name"><b> Author Designation</b></label>    
                <input class="form-control" type="text" name='a_desi' value="<?php 
                if (isset($_GET['id']) && $_GET['id'] != '') {
                echo $row_display['a_desi']; 
                }
                ?>">
            </div>
            ...
            <label for="name"><b> Date</b></label>    
                ...
                if (isset($_GET['id']) && $_GET['id'] != '') {
                echo $row_display['date']; 
                }
                ?>">
            </div>
            <div class="col-12 mb-30">
            <label for="name"><b> Author Description</b></label> 
                <textarea class="form-control" name='a_desc'><?php
                if (isset($_GET['id']) && $_GET['id'] != '') {
                echo $row_display['a_desc']; 
                }
                ?></textarea>
            </div>
            ...
            <label for="name"><b> Description</b></label> 
                <textarea class="form-control" id="description" name='b_desc' ><?php
                if (isset($_GET['id']) && $_GET['id'] != '') {
                echo $row_display['b_desc']; 
                }
                ?></textarea>
            </div> 
            ...
            <label for="name"><b> Category </b></label> 
                <select class="form-control nice-select" name='categories' required>
                    ...
                    while ($row = mysqli_fetch_assoc($result) ){
                        if ($row['id'] == $row_display['category']) {
                            echo "
